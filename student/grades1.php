...
<table cellpadding="5" cellspacing="0" id="customers" border="1" style="font-size:15px">
  ...
  <?php
		...
		$grade1 = 0;
	$result= mysql_query("SELECT * FROM quiz WHERE idnumber = '$username'");
	while($row = mysql_fetch_array($result))
			{
			$subject = $row['subject'];	
			echo '<tr>';
		...
		//check for grades
		echo '<td>';
		$result4= mysql_query("SELECT * FROM quiz WHERE idnumber = '$username' AND term ='First Semester' OR quarter = 'First Quarter'  ORDER BY ID DESC LIMIT 1");
		...
		echo '<td>';
		$result4= mysql_query("SELECT * FROM quiz WHERE idnumber = '$username' AND term ='Second Semester' OR quarter = 'Second Quarter'  ORDER BY ID DESC LIMIT 1");
		$count=mysql_num_rows($result4);
		if($count>0) {
		while($row4 = mysql_fetch_array($result4))
			{
...
<table cellpadding="5" cellspacing="0" id="customers" border="1" style="font-size:15px">
  ...
  <?php
	...
	while($row = mysql_fetch_array($result))
		...
		//check for grades
		echo '<td>';
		$result4= mysql_query("SELECT * FROM quiz WHERE idnumber = '$username' AND term ='Second Semester' OR quarter = 'First Quarter' ORDER BY ID DESC LIMIT 1");
		$count=mysql_num_rows($result4);
		if($count>0) {
		while($row4 = mysql_fetch_array($result4))
			{
