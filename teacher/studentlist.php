...
<table cellpadding="1" cellspacing="1" id="customers" style="font-size:12px">
	...
	$results = mysql_query("SELECT * FROM teacher WHERE idnumber='$idnumber'");
			...
			$result1 = mysql_query("SELECT * FROM seniorsubjects WHERE subject='$subject' ORDER by strand ASC");
			...
			while($row1c = mysql_fetch_array($result1c))
				{
					...
					echo '<td><div align="center"><a href="student_list.php?subject='.$row['subject'].'&type=Senior High School&grade='.$row1c['grade'].'&section='.$row1c['strand'].'" title="Click To Edit" class="btn btn-info">View Students</a>  </div></td>';
					echo '</tr>';
