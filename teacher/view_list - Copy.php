...
<h2>Submit Grade - <?php echo $_GET['quarter'] ?> : <?php echo $_GET['grade'].' '.$_GET['section'] ?></h2>
   ...
   <!--start-->
			...
			<input type="search" class="light-table-filter email" data-table="order-table" placeholder="Search" style="border:0.5px solid #d5d5d5;padding:5px">
			<br>
<table id="customers" class="tftable order-table table bluetext">
			<thead>
			  <tr>
				...
				$results = mysql_query("SELECT * FROM juniorstudents WHERE course = '$grade' AND section='$section'");
						while($rows = mysql_fetch_array($results))
							{	
						$student = $rows['idnumber'];
						$results1 = mysql_query("SELECT * FROM quiz WHERE subject='$subject' AND idnumber='$student' AND term = '$quarter' ORDER BY ID DESC LIMIT 1");
			  ...
			  <tr>
				...
				<td width=""><span style="float:left"><?php echo $rows['lname'].' '.$rows['fname'].' '.$rows['mname']?></td>
				
				<td width="8%" align="center" >
<input type="hidden" name="term" required  class="form-control" style="width:200px" value="<?php echo $_GET['quarter'] ?>">				
				<input type="number" name="grade" required  class="form-control" style="width:200px" value="<?php echo $grade ?>">
