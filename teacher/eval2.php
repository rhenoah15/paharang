...
@keyframes fade {
  ...
  to {opacity: 1}
	...
	<?php
	$idnumber1 = $_GET['id'];
				$result = mysql_query("SELECT * FROM teacher WHERE idnumber= '$idnumber1'");
	...
	<table id="customers">
		...
		<tbody>
			<?php
				$resulta = mysql_query("SELECT * FROM question WHERE type = 'P2P' GROUP BY category");
		...
		echo '</tr>';
				$resulta1 = mysql_query("SELECT * FROM question WHERE category = '$category' AND type = 'P2P'");
		...
		//check for answer
		$faculty  =$_GET['id'];
		$result1 = mysql_query("SELECT * FROM evaluation WHERE faculty = '$faculty' AND student = '$idnumber' AND qid = '$id1' ");
