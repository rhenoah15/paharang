...
<tr><td width="20%">
		...
		<select name="grade" required class="form" onChange="showUser(this.value)" id="grade">
			<option></option>
			<?php
		$results = mysql_query("SELECT * FROM senior GROUP BY grade");
		...
		<tr>
			<td>Strand</td>
			<td>
		<select name="section" required class="form"  id="section" onchange="showUser1(this.value)">
			<option></option>
		</select></td>
		<td></td>
		</tr>
		<tr>
			<td>Subject</td>
			<td>
		<select name="subject" class="form" required id="subject">
		<option></option>
		<?php
...
function showUser1(str) {
  ...
  //alert(str);
  xmlhttp.open("GET","g1a.php?q="+str+"&grade="+grade+"",true);
  xmlhttp.send();
